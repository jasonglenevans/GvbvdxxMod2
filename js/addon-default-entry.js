  ...
  ScratchBlocks.BlockDragger.prototype.startBlockDrag = function () {
    if (!addon.self.disabled) {
      ...
      if (ctrlKeyPressed === !invert) {
        ...
        this.draggingBlock_.unplug(true); // A separate field has to be updated to avoid dragging comments attached to blocks underneath this block.

        this.dragIconData_ = this.dragIconData_.filter(i => i.icon.block_ === this.draggingBlock_); // And we also have to make sure Scratch will never try to drag a block on top of itself, which crashes the editor.
        // That can happen when you cherry-pick the top block of a script and drop it where it started.

        this.draggedConnectionManager_.availableConnections_ = this.draggedConnectionManager_.initAvailableConnections_();
      }
    }
