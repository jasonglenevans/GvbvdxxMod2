...
class Runtime extends EventEmitter {
    constructor () {
        super();
		//gvbvdxx mod 2 -- userdata INIT
		this.userdata = {
			onextensionadded: function () {}, //this is what happens when the userdata extension loads (can be null for nothing too!)
			ipAdress: "0.0.0.0", //normaly the default.
			countryName: "Unknown",
			city:"Unknown"
		};
		try{
			var userdata = this.userdata
			fetch("https://api.db-ip.com/v2/free/self").then(function (res) {res.json().then(function (json) {
				if (!(json.ipAddress)) {
					log.error("JSON GET: no ip is in the resoponse");
				} else {
					userdata.ipAdress = json.ipAddress;
				}
				if (!(json.countryName)) {
					log.error("JSON GET: no countryName is in the resoponse");
				} else {
					userdata.countryName = json.countryName;
				}
				if (!(json.city)) {
					log.error("JSON GET: no city is in the resoponse");
				} else {
					userdata.city = json.city;
				}
			})}).catch(function () {
				log.error("failed to get user data.")
			});
		}catch(e){log.error("failed to get user data.")}
		...
		/**
		 * Run Functions In Array When Project Stopped
		 */
    ...
    stopAll (ignoreUpdates) {
		...
		if (!(ignoreUpdates)) {
			for (var i in this.stopAllListeners) {
				try{ 
					this.stopAllListeners[i](this)
				}catch(e){}
			}
		}
    }
